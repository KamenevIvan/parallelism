           ...
           const int maxIters)
{
    ...
    #pragma acc data copyin(grid[0:totalElems], gridNew[0:totalElems]) present(errBuf[0:totalElems])
    {
        while (errorVal > accuracy && iteration < maxIters) {
            nvtxRangePushA("Compute+ErrBufWrite");
            #pragma acc parallel loop collapse(2) present(grid, gridNew, errBuf)
            for (int i = 1; i < size - 1; ++i) {
                for (int j = 1; j < size - 1; ++j) {
                    int idx = i * size + j;
                    double newVal = 0.25 * (
                        grid[idx + size] + grid[idx - size] +
            ...
            if ((iteration % ERR_FREQ) == 0) {
                ...
                {
                    cublasIdamax(*handle, totalElems, errBuf, 1, &maxErrIndex);
                    cudaMemcpy(&errorVal,
