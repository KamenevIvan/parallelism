...
void initialize_grid(double* __restrict__ grid, double* __restrict__ new_grid, size_t size) {
    ...
    #pragma acc enter data copyin(grid[0:size*size], new_grid[0:size*size]) async 
...
double calculate_next_grid(double* grid, double* new_grid, size_t size) {
    ...
    #pragma acc parallel loop collapse(2) vector vector_length(256) gang num_gangs(256) reduction(max:error) present(grid, new_grid) async 
    for (int i = 1; i < size - 1; ++i) {                                                  
...
void copy_grid(double* grid, const double* new_grid, size_t size) {
    #pragma acc parallel loop collapse(2) present(grid, new_grid) async
    for (int i = 1; i < size - 1; ++i) {
        for (int j = 1; j < size - 1; ++j) {
...
int main(int argc, char* argv[]) {
    ...
    while (error > accuracy && iter < max_iterations) {
        ...
        if (iter % 10 == 0) error = calculate_next_grid(grid, new_grid, size);
        else calculate_next_grid(grid, new_grid, size);
        nvtxRangePop();
